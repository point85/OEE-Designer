...
public class LeaderBoardTileSkin extends TileSkin {
	...
	protected void initGraphics() {
		...
		List<LeaderBoardItem> leaderBoardItems = tile.getLeaderBoardItems().stream()
				.sorted(Comparator.comparing(LeaderBoardItem::getValue).reversed()).collect(Collectors.toList());
	...
	private void registerItemListeners() {
		tile.getLeaderBoardItems().forEach(item -> {
			...
			EventHandler<MouseEvent> clickHandler = e -> tile
					.fireTileEvent(new TileEvent(TileEvent.EventType.SELECTED_CHART_DATA, item.getChartData()));
